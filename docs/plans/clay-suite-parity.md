...
| --- | --- | --- | --- |
| Tables and enrichment | Strong workbook, waterfalls, formulas, AI/research and output columns; reactive dependency controls; global saved-view filter/sort/search before pagination; matching-query run/fill and cost estimates; formula-safe streaming export; count-locked delete-all-matching; versioned stable keyset cursors for default and custom saved-view ordering; server-paged virtualized keyboard navigation; Shift-range copy and fill-down; atomic multi-row TSV paste; horizontally safe queue with bounded parallel slots, indexed tenant ownership, advisory-lock-enforced tenant backpressure/fair claiming, graceful drain, admin telemetry, and a repeatable controlled-load gate | Execute the load gate on PostgreSQL at supported scale, add indexed JSON search/sort expressions and arbitrary cross-page selection at million-row scale, and broaden provider coverage | P0 |
