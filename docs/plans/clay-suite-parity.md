...
| --- | --- | --- | --- |
| Tables and enrichment | Strong workbook, waterfalls, formulas, AI/research and output columns; reactive dependency controls; virtualized keyboard navigation; Shift-range copy and fill-down; atomic multi-row TSV paste; horizontally safe queue with bounded parallel slots, indexed tenant ownership, advisory-lock-enforced tenant backpressure/fair claiming, graceful drain, admin telemetry, and a repeatable controlled-load gate | Execute the load gate on PostgreSQL at supported scale, add server-driven navigation beyond the first 1,000 visible rows, and broaden provider coverage | P0 |
