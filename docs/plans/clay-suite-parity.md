...
| Governance | Workspace member lifecycle and capability policies; RLS; tenant-bound LLM usage reporting; append-only mutation audit with bounded secret-stripping action metadata; retention; tenant-bound OIDC SSO; workspace-scoped SCIM 2.0 User and Group synchronization with hashed rotatable tokens; audited workspace-scoped credentials across CRM, sequencer, Airtable and Google Sheets outputs | Add the shared PostgreSQL LLM usage ledger and complete controlled-live IdP, directory, and connector validation | P2 |
